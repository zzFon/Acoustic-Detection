function mix=gmm_num(ncentres,data,kiter,covar_type)
    %% 输入：
    % ncentres:混合模型数目
    % train_data:训练数据
    % kiter:kmeans的迭代次数
...
max_ncentres = 15;
err_list = [];
for ncentres = 1:max_ncentres%遍历核数目
    ...
    [mix.centres,post,err]=k_means(mix.centres,data,kiter);
    fprintf('centres = %d, error = %d\n',ncentres,err);
...
figure(1);
subplot(2,1,1);plot(err_list);xlabel('number of centers');ylabel('WSS');
subplot(2,1,2);plot(err_list(2:end)-err_list(1:end-1));xlabel('number of centers');ylabel('change of WSS');

end
%=============================================================
function [centres,post,error]=k_means(centres,data,kiter)
    [dim,data_sz]=size(data');
